    ...
    const { data: participationsData } = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await fetch(
                `${apiUrl}/me/participations?walletAddress=${walletAddress.toLowerCase()}`
            );
        ...
        const checkCompletedRounds = async () => {
            ...
            for (const participation of participationsData.participations) {
                ...
                if (
                    ...
                    isRoundViewed(participationChainId, roundId)
                ) {
                    continue;
                }
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        // Only show if round is completed (CLOSED status), user participated, AND user won
                        if (
                            result.participated &&
                            result.roundStatus === "CLOSED" &&
                            result.isWinner && // Only show modal for winners
                            participationChainId
                        ) {
                    ...
                    console.error(
                        `Error fetching result for round ${roundId}:`,
                        error
                    );
                }
            }
    ...
    return {
        ...
        clearResult: (roundId: number) => {
            setPendingResults((prev) =>
                prev.filter((r) => r.roundId !== roundId)
            );
        },
    };
}
